...
* When rasterizing a feature collection comprised of points, most cells in the resulting layer are likely to have no data, unless the points are densely packed or the cell size (determined by the number of cells in the ``rasterWidth`` and ``rasterHeight`` parameters) is large. To create a grid coverage with values in all cells by using interpolation methods, use an interpolation process such as :ref:`gs:BarnesSurface <processing.processes.vector.barnessurface>`.
